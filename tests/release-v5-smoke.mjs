...
const notes = fs.readFileSync(new URL('../RELEASE_NOTES_v5.11.5.md', import.meta.url), 'utf8');
...
assert.match(indexHtml, /const APP_VERSION = '5\.11\.5';/);
...
assert.match(indexHtml, /collaboration-core-v5\.js\?v=5\.11\.5/);
assert.match(indexHtml, /live-collaboration-v5\.js\?v=5\.11\.5/);
assert.match(indexHtml, /app-v5\.js\?v=5\.11\.5/);
assert.match(indexHtml, /timeline-core-v5\.js\?v=5\.11\.5/);
...
assert.match(scannerHtml, /processing\/lightweight\.js\?v=5\.11\.5/);
...
assert.equal(update.publishedAppVersion, '5.11.5');
assert.equal(update.version, '1.0.10');
assert.equal(update.versionCode, 11);
assert.equal(update.apkSizeBytes, 3159597);
assert.match(update.releaseNotes, /v5\.11\.5/);
assert.match(notes, /four detected corners/i);
assert.match(notes, /perspective mesh/i);
assert.match(notes, /before-and-after wipe/i);

console.log('v5.11.5 smoke checks passed.');
