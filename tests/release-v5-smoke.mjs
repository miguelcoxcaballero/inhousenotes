...
const notes = fs.readFileSync(new URL('../RELEASE_NOTES_v5.11.13.md', import.meta.url), 'utf8');
...
assert.match(indexHtml, /const APP_VERSION = '5\.11\.13';/);
...
assert.match(indexHtml, /collaboration-core-v5\.js\?v=5\.11\.13/);
assert.match(indexHtml, /live-collaboration-v5\.js\?v=5\.11\.13/);
assert.match(indexHtml, /app-v5\.js\?v=5\.11\.13/);
assert.match(indexHtml, /timeline-core-v5\.js\?v=5\.11\.13/);
...
assert.match(scannerHtml, /processing\/lightweight\.js\?v=5\.11\.13/);
...
assert.equal(update.publishedAppVersion, '5.11.13');
assert.equal(update.version, '1.0.10');
assert.equal(update.versionCode, 11);
assert.equal(update.apkSizeBytes, 3159597);
assert.match(update.releaseNotes, /v5\.11\.13/);
assert.match(notes, /adapts the yellow threshold/i);
assert.match(notes, /local colour contrast/i);
assert.match(notes, /four printed colour circles/i);
assert.match(notes, /narrow and broad strip fits/i);
assert.match(notes, /six supplied photos/i);
assert.match(notes, /16 of 16/i);

console.log('v5.11.13 smoke checks passed.');
