...
const notes = fs.readFileSync(new URL('../RELEASE_NOTES_v5.11.10.md', import.meta.url), 'utf8');
...
assert.match(indexHtml, /const APP_VERSION = '5\.11\.10';/);
...
assert.match(indexHtml, /collaboration-core-v5\.js\?v=5\.11\.10/);
assert.match(indexHtml, /live-collaboration-v5\.js\?v=5\.11\.10/);
assert.match(indexHtml, /app-v5\.js\?v=5\.11\.10/);
assert.match(indexHtml, /timeline-core-v5\.js\?v=5\.11\.10/);
...
assert.match(scannerHtml, /processing\/lightweight\.js\?v=5\.11\.10/);
...
assert.match(pagesWorkflow, /timeout-minutes: 38/);
assert.match(pagesWorkflow, /Date\.now\(\) \+ 35 \* 60 \* 1000/);
...
assert.equal(update.publishedAppVersion, '5.11.10');
assert.equal(update.version, '1.0.10');
assert.equal(update.versionCode, 11);
assert.equal(update.apkSizeBytes, 3159597);
assert.match(update.releaseNotes, /v5\.11\.10/);
assert.match(notes, /lower square box/i);
assert.match(notes, /bright yellow/i);
assert.match(notes, /four printed colour circles/i);
assert.match(notes, /brush-matched ink tones/i);
assert.match(notes, /35 minutes/i);
assert.match(notes, /single production queue/i);

console.log('v5.11.10 smoke checks passed.');
